	...
	define([], function(){
		...
		function runTests(context){
			...
			tests.forEach(function(test){
				try{
					test.fn(context);
					console.log((test.name + repeat(' ', (50 - test.name.length)) + ': Passed').green.bold);
				}catch(e){
					var j = 0,
						err;
		...
		nsjstests.runTests = runTests;
		return nsjstests;
	});
}(
(typeof define !== "undefined")?define
:(typeof exports !== "undefined")?function(dep, factory){module.exports = factory();} 
: function(dep, factory){ this.nsjstests = factory(); })
);